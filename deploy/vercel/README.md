...
Vercel automatically discovers the `api/index.ts` file, which exports the Elysia server as the default export. The `vercel.json` configures Bun as the runtime.
...
> **Note**: Serverless functions have execution time limits (10-60s depending on plan). Long-running background jobs via Trigger.dev may need external hosting.
...
### Build Errors
Ensure all dependencies are listed in `package.json`. Vercel uses `bun install` by default.
...
### Additional Help
Refer to [Vercel Elysia Documentation](https://vercel.com/docs/frameworks/backend/elysia)
